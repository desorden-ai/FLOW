...
function parseLogos(section: HTMLElement | null): ParsedLogo[] {
  if (!section) return [];
  return Array.from(section.querySelectorAll<HTMLImageElement>("[data-logo-3d-item]")).map((logo, index) => ({
    element: logo,
...
function renderIntro(intro: HTMLElement, progress: number) {
  const introOpacity = clampFinite(1 - progress / INTRO_END, 0, 1, 0);
  const introShift = progress * -100;
  intro.style.opacity = String(introOpacity);
  intro.style.transform = `translate3d(-50%, ${introShift}px, 0)`;
}

function renderLogos(logos: ParsedLogo[], progress: number) {
  const logosEnabled = progress > INTRO_END;
  const logoProgress = clampFinite((progress - INTRO_END) / (1 - INTRO_END), 0, 1, 0);
  const currentZAdvance = logoProgress * TOTAL_Z_TRAVEL;

  for (const { element: logo, initialZ, flowDirection, flowPhase } of logos) {
...
function createLogoTunnelController({
  section,
  root,
  intro,
  logos,
  ...
  const render = (progress: number) => {
    ...
    renderIntro(intro, safeProgress);
    renderLogos(logos, safeProgress);
  };
...
export function useLogoTunnelAnimation(sectionRef: RefObject<HTMLElement | null>) {
  useEffect(() => {
    ...
    const logos = parseLogos(section);
    ...
    const controller = createLogoTunnelController({
      section,
      root,
      intro,
      logos,
